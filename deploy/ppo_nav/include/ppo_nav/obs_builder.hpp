...
struct PlannerParams {
  double range_max_norm = 7.0;    // 归一化量程（训练值，lidar 用 clip(r,0,rmax)/rmax）
  int num_beams = 100;            // 训练 lidar 光束数
  double half_fov_rad = M_PI / 2.0;  // 前向半视场（训练 angle_range=pi）
  double goal_dist_norm = 10.0;   // goal 距离归一化除数（训练 obs.goal_dist_norm）
  ...
  int chunk_size = 1;             // action chunk 长度 N（N=1 或 N=5）
...
///       [num_beams+3:]=上一动作（N*action_dim，PPO 训练时恒为 0，见 setPrevChunk 注释）。
class ObsBuilder {
 public:
  ...
  /// 由机器人位姿 (x,y,yaw) 生成 goal 3 维特征 [dist/goal_dist_norm, cos(a), sin(a)]。
  void buildGoal(double rx, double ry, double ryaw, std::vector<float>& out) const;

  /// 组装完整 obs（lidar + goal + prev），长度应为 num_beams+3+N*action_dim（N=1: 105，N=5: 113）。
  void buildObs(const sensor_msgs::LaserScan& scan, double rx, double ry, double ryaw,
                std::vector<float>& obs) const;

  /// 设置上一动作（归一化 [-1,1]，逐维按训练变换 [lin*2, (ang+1)/2]，不足补零、多余截断）。
  /// ⚠️ 当前 PPO 模型训练走 step_single，训练侧"上一动作"通道恒为 0 —— 因此部署默认
  ///    不要调用本方法（prev 保持全零，与训练分布一致）；只有用带 prev 反馈重新训练的
  ///    模型时才启用（配合 planner 参数 use_prev_action: true）。
